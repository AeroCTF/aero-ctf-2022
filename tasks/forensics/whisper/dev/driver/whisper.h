...
typedef struct channel_list_elem {
    channel_t * cur;
    struct channel_list_elem * next;
} channel_elem_t;

channel_elem_t * pChannelList = NULLPTR;


static noinline size_t create_channel(unsigned long);
static noinline size_t destroy_channel(unsigned long);
static noinline size_t add_message_to_channel(unsigned long);
static noinline size_t get_message_from_channel(unsigned long);
...
static noinline size_t dec_aes(channel_t*);
static noinline size_t dec_rc4(channel_t*);
static noinline size_t enc_aes(channel_t*);
static noinline size_t enc_rc4(channel_t*);

static noinline void swaps(unsigned char *a, unsigned char *b);
static noinline size_t KSA(uint8_t *, unsigned char *, size_t );
static noinline size_t PRGA(unsigned char*, uint8_t*,uint8_t*,size_t);
static noinline size_t RC4(uint8_t*, uint8_t *, uint8_t *, size_t, size_t);

static noinline long ioctl_handler(struct file *, unsigned int, unsigned long);
static struct file_operations whisper_fops = {.unlocked_ioctl = ioctl_handler}; 

struct miscdevice whisper_dev = {
